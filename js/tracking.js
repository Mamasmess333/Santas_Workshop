...
class TrackingSystem {
   ...
   */
  async recordCompletion(puzzleSize, timeSeconds, moves, completed = true) {
    if (!this.currentSessionId) {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        // Only save metrics and update behavior if completed is true
        if (completed) {
  ...
  async savePerformanceMetrics(timeSeconds, moves) {
    ...
    const optimalMoves = this.estimateOptimalMoves(
      window.gameController ? window.gameController.currentSize : 4
    );
    ...
    try {
      await fetch("php/tracking.php", {
        ...
        body: JSON.stringify({
          ...
          difficulty_adjustment: window.adaptiveSystem
            ? window.adaptiveSystem.getSkillLevel()
            : 1.0,
        }),
      });
    } catch (error) {
  ...
  async getGameHistory(limit = 10) {
    try {
      const response = await fetch(
        `php/tracking.php?action=get_history&limit=${limit}`
      );
      const data = await response.json();
