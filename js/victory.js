...
class VictorySystem {
  ...
  showVictory(timeSeconds, moves) {
    ...
    if (window.trackingSystem) {
      const puzzleSize = window.gameController
  ...
  handleNextLevel() {
    ...
    // Show story modal for new level if needed
    if (nextSize && window.storySystem) {
      setTimeout(() => {
        window.storySystem.onPuzzleSizeChange(nextSize);
      }, 500);
    }
