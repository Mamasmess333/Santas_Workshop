...
class PowerUpSystem {
   ...
   */
  async unlockPowerUp(powerUpType, quantity = 1) {
    ...
    if (typeof this.powerUps[powerUpType].available === "number") {
      this.powerUps[powerUpType].available += quantity;
    } else {
      this.powerUps[powerUpType].available = quantity;
    }
    this.renderPowerUps();
  ...
  useTimeFreeze() {
    if (
      !this.powerUps.timeFreeze.unlocked ||
      this.powerUps.timeFreeze.available <= 0
    ) {
      return false;
    }
  ...
  useExtraHints() {
    if (
      !this.powerUps.extraHints.unlocked ||
      this.powerUps.extraHints.available <= 0
    ) {
      return false;
    }
  ...
  useMoveUndo() {
    if (
      !this.powerUps.moveUndo.unlocked ||
      this.powerUps.moveUndo.available <= 0
    ) {
      return false;
    }
  ...
  useTilePreview() {
    if (
      !this.powerUps.tilePreview.unlocked ||
      this.powerUps.tilePreview.available <= 0
    ) {
      return false;
    }
  ...
  useSpeedBoost() {
    if (
      !this.powerUps.speedBoost.unlocked ||
      this.powerUps.speedBoost.available <= 0
    ) {
      return false;
    }
  ...
  useShuffleProtection() {
    if (
      !this.powerUps.shuffleProtection.unlocked ||
      this.powerUps.shuffleProtection.available <= 0
    ) {
      return false;
    }
    ...
    if (shuffleBtn) {
      ...
      shuffleBtn.onclick = () => {
        if (
          confirm(
            "Are you sure you want to shuffle? This will reset your progress."
          )
        ) {
    ...
    const powerUpTypes = [
      {
        ...
        name: "Time Freeze",
        icon: "⏸️",
        action: () => this.useTimeFreeze(),
      },
      {
        key: "extraHints",
        name: "Extra Hints",
        icon: "💡",
        action: () => this.useExtraHints(),
      },
      {
        key: "moveUndo",
        name: "Undo Move",
        icon: "↩️",
        action: () => this.useMoveUndo(),
      },
      {
        key: "tilePreview",
        name: "Tile Preview",
        icon: "👁️",
        action: () => this.useTilePreview(),
      },
      {
        key: "speedBoost",
        name: "Speed Boost",
        icon: "⚡",
        action: () => this.useSpeedBoost(),
      },
      {
        key: "shuffleProtection",
        name: "Shuffle Protection",
        icon: "🛡️",
        action: () => this.useShuffleProtection(),
      },
    ];
